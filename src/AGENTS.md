...
- `MeshRepair.cpp` — RemoveDuplicate/Degenerate faces, FixNonManifold,
  ...
  RemoveSpikes. Repair entry points that work on both representations dispatch
  to an `*Arrays` / `*HalfEdge` arm.
...
- `GltfImageCodec.cpp` / `.h` — the OpenCV-backed `GltfLoadImageData` / `GltfWriteImageData`
  tinygltf callbacks (tinygltf is built with `TINYGLTF_NO_STB_IMAGE[_WRITE]`, so it ships
  no codec of its own), plus `SetGltfImageCodec()`, which every `tinygltf::TinyGLTF`
  context must call before touching images (`MeshIO.cpp` and `tests/MeshIOTest.cpp` do).
  This is also the single TU that `#define`s `TINYGLTF_IMPLEMENTATION` — the two are fused
  because the writer needs tinygltf helpers that exist only behind that macro.
