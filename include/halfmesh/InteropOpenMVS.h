...
void ConvertFromMVS(Src& src, halfmesh::Mesh& dst)
{
	...
	// Authored per-vertex normals ride across under the same size guard. halfmesh
	// never recomputes them: it remaps them through the operations that only
	// renumber vertices and clears them outright in the ones that move a vertex,
	// so what comes back is either the caller's own values or nothing.
	if (src.vertexNormals.size() == numVertices) {
		dst.vertexNormals.resize(src.vertexNormals.size());
		for (size_t i = 0; i < dst.vertexNormals.size(); ++i) {
			const MVS::Mesh::Normal& n = src.vertexNormals[(MVS::Mesh::VIndex)i];
			dst.vertexNormals[i] = halfmesh::Mesh::Normal(n.x, n.y, n.z);
		}
	}
	if constexpr (kRelease)
		src.vertexNormals.Release();
...
void ConvertToMVS(Src& src, MVS::Mesh& dst)
{
	...
	// Authored per-vertex normals, when the operation preserved them (see the
	// MVS -> halfmesh note); an empty array means the caller has to derive them.
	if (src.vertexNormals.size() == numVertices) {
		dst.vertexNormals.resize((MVS::Mesh::VIndex)src.vertexNormals.size());
		for (size_t i = 0; i < src.vertexNormals.size(); ++i) {
			const halfmesh::Mesh::Normal& n = src.vertexNormals[i];
			dst.vertexNormals[(MVS::Mesh::VIndex)i] = MVS::Mesh::Normal(n[0], n[1], n[2]);
		}
	}
	if constexpr (kRelease)
		ReleaseVector(src.vertexNormals);
...
// Convert an openMVS mesh into a halfmesh mesh (geometry, per-vertex colors and
// normals, per-face normals, per-corner UVs, and diffuse textures).  Only the
// fields halfmesh understands are transferred; openMVS extras (vertex/face
// adjacency caches, octree, etc.) are not copied — those are derived data: call
...
// Convert a halfmesh mesh into an openMVS mesh (geometry, per-vertex colors and
// normals, per-face normals, per-corner UVs, and diffuse textures).
